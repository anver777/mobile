...
export function GoalsSection({ initialGoals }: { initialGoals: Goal[] }) {
  ...
  async function toggleGoal(goal: Goal) {
    ...
    setGoals((prev) =>
      prev.map((g) =>
        g.id === goal.id ? { ...g, completed: next, updatedAt: new Date() } : g
      )
    );
    try {
  ...
  return (
    <>
      {/* HEADER */}
      <header className="relative shrink-0 overflow-hidden px-5 pb-8 pt-8 text-white" style={headerBg}>
        <div
        ...
        <div className="relative mx-auto max-w-xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-base font-extrabold tracking-wide" style={{ textShadow: `0 0 14px rgba(${tf.glowRgb},0.6)` }}>
                Мои цели
              </span>
            </div>
            <button
          ...
          <div className="mt-7 flex items-end justify-between gap-4">
            <div className="min-w-0">
              <p className="text-sm font-medium text-white/55">{periodSubtitle(active)}</p>
              <h1
              ...
              <p className="mt-2 text-[13px] font-semibold text-white/70">
                {total === 0 ? "Поставьте первую цель ✨" : `${completed} из ${total} выполнено`}
              </p>
            </div>
            <ProgressRing progress={progress} color={tf.accent} size={108}>
              <span className="text-2xl font-extrabold leading-none" style={{ color: tf.accent, textShadow: `0 0 14px rgba(${tf.glowRgb},0.7)` }}>
      ...
      <main className="flex-1 px-4 py-4 pb-6">
        <PullToRefresh onRefresh={refresh} color={tf.accent}>
          <div className="mx-auto max-w-xl">
            {error && (
              <div className="mb-3 flex items-center justify-between rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-3 text-sm font-medium text-red-300">
                {error}
            ...
            {total === 0 && !loading ? <GoalsEmpty tf={tf} onAdd={openNew} /> : (
              <div className="flex flex-col gap-2.5">
                ...
                {allDone && total > 0 && (
                  ...
                  >
                    ...
                    <p className="mt-1 text-base font-bold text-[#00ffa3]" style={{ textShadow: "0 0 12px rgba(0,255,163,0.6)" }}>
                      Все цели выполнены!
                    </p>
      ...
      <button
        ...
        style={{
          ...
          boxShadow: `0 0 32px rgba(${tf.glowRgb},0.7), 0 10px 30px rgba(0,0,0,0.4)`,
